...
let rec seq_length (xs : 'a seq) : int =
...
let int_seq_length (seed : int) : int = seq_length (Cons (seed, Nil))
[@@verocaml.spec]

let bool_seq_length (seed : bool) : int = seq_length (Cons (seed, Nil))
[@@verocaml.spec]
...
let rec node_length (value : node) : int =

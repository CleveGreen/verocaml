The production registry matrix is an explicit private-authority exception.
Its exact atomic-batch and zero-delta counters are outside the closed semantic
projection and cannot block finite-formal-propagation-outcome-check.
...
The retained-ABI matrix and fixture-parser failures are explicit specialist
input/authority exceptions.  They guard pre-session identity and zero-delta
behavior rather than user-visible diagnostic presentation.
...
Exact private receipt counters for accepted flows are specialist resource
exceptions; grouped outcomes own their verified status and named functions.

  $ retained positive
  $ trace artifacts/positive.cmt
  formal-counters assumptions=4 batches=6 transfers=8 consumptions=6

The private production-driver dispatch count remains a specialist authority
exception.
...
Mode and recursive-transfer counters are specialist authority/resource
exceptions.  The grouped host owns accepted status, functions, and recursive
descent; SST/VIR render and generated-identity assertions were removed.

  $ for n in same_cmt_modes generic_recursive_proof push_preservation; do retained "$n"; done
  ...
  $ VEROCAML_TEST_PRIVATE_RECEIPT_TRACE=1 DELATOR_LOG=Verification_session=debug,warn DELATOR_FORMAT=flat DELATOR_COLOR=never OCAML_COLOR=never ../../src/verocaml.exe verify artifacts/generic_recursive_proof.cmt --timeout-ms 5000 2>&1 | grep 'Verification_session: private receipt event_kind=lower ' | sed -E 's|.*function_name=([^ ]+) function_index=([0-9]+).*finite_formal_assumption_issuances=([0-9]+) finite_formal_transfer_batches=([0-9]+) finite_formal_transfers=([0-9]+) finite_formal_transfer_consumptions=([0-9]+).*|checkpoint function=\1#\2 counters=\3/\4/\5/\6|; s/#[0-9]+/#N/'
  checkpoint function=lemma_equal_refl#N counters=0/0/0/0
  checkpoint function=int_instantiation#N counters=1/1/1/1
  checkpoint function=bool_instantiation#N counters=2/2/2/2
  $ trace artifacts/push_preservation.cmt
  formal-counters assumptions=1 batches=1 transfers=1 consumptions=1

CMI/CMTI presence, adjacent-interface authentication, and pre-session trace
absence are explicit specialist input-mode exceptions.  Stable acceptance and
VERO codes also have grouped Outcome replacements.
  ...
  $ verify_public_generic () { label=$1; directory=$2; name=$3; OCAML_COLOR=never ../../src/verocaml.exe verify "$directory/$name.cmt" --timeout-ms 5000 --dump-sst "$directory/accepted.sst" >/dev/null; test "$(grep -Ec '^function seq_reflexive#[0-9]+ binders=' "$directory/accepted.sst")" = 1; test "$(grep -c '^type seq<' "$directory/accepted.sst")" = 0; printf '%s: verified canonical-schema=true clones=0\n' "$label"; }
...
A constrained same-CMT module's private transfer count and the reordered
interface malformed-SST barrier are specialist exceptions.  Stable positive
status and add/remove rejection codes moved to grouped Outcomes.
...
Retained private-transfer counters and interface payload non-leakage are
specialist authority/input exceptions.  Grouped Dune-project Outcomes own
provider/consumer verification, functions, and call-precondition semantics.
  ...
  $ VEROCAML_TEST_PRIVATE_RECEIPT_TRACE=1 DELATOR_LOG=Verification_session=debug,warn DELATOR_FORMAT=flat DELATOR_COLOR=never OCAML_COLOR=never ../../src/verocaml.exe verify artifacts/retained_simple_bindings_provider.cmt --timeout-ms 5000 >artifacts/retained-provider.out 2>artifacts/retained-provider.trace
  ...
  checkpoint function=seq_reflexive#N counters=0/0/0/0
  checkpoint function=int_seq_reflexive#N counters=1/1/1/1
  checkpoint function=bool_seq_reflexive#N counters=1/2/2/2
  ...
  $ OCAML_COLOR=never ../../src/verocaml.exe verify artifacts/retained_simple_bindings_consumer.cmt --dependency artifacts/retained_simple_bindings_provider.cmt --timeout-ms 5000 --dump-sst artifacts/retained-consumer.sst >/dev/null
  $ if grep 'seq_' artifacts/retained-consumer.sst; then exit 1; fi

The CMI-only path is a specialist input-mode exception.  It retains CMTI
absence, private transfer counts, and metadata non-leakage without comparing
complete SST/VIR renders.
  ...
  $ OCAML_COLOR=never ../../src/verocaml.exe verify artifacts/retained-cmi-only/retained_simple_bindings_consumer.cmt --dependency artifacts/retained-cmi-only/retained_simple_bindings_provider.cmt --timeout-ms 5000 --dump-sst artifacts/retained-cmi-only/consumer.sst >/dev/null
  $ if grep 'seq_' artifacts/retained-cmi-only/consumer.sst; then exit 1; fi
The retained arbitrary consumer is an explicit malformed-SST outcome-gap
exception.
...
The projected call-precondition counterexample moved to the grouped host; its
private transfer count remains a specialist authority exception.

  $ retained false_precondition_after_transfer
  $ trace artifacts/false_precondition_after_transfer.cmt
  formal-counters assumptions=1 batches=1 transfers=1 consumptions=1

Arbitrary entry and one-path-only malformed-SST barriers are specialist
outcome-gap exceptions with exact no-transfer counters.
...
Missing default-Ghost receipts are specialist malformed-SST outcome-gap
exceptions with exact no-transfer counters.
...
Pre-CMT PPX syntax rejection is an explicit specialist input exception; no
selected CMT exists for a VERO-113 fixture Outcome.
...
Source-written internal marker rejection and output cleanup are specialist
input/process exceptions outside the semantic fixture seam.
...
Implementation/signature erasure and binary metadata presence are specialist
compiler-artifact exceptions.  Complete SST/VIR repeatability and rendered
finite-formal rows were removed.

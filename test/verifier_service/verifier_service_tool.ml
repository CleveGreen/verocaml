...
let provenance_matrix consumer dependency =
  ...
  let provider =
    ...
    | None -> fail "expected provider provenance"
  in
  require
    (String.equal
       (Verifier_service.provenance_unit_name provider)
  ...
  print_endline "provenance=unit/digest/direct/transitive status=verified"
...
let trusted_matrix consumer =
  ...
  List.iter
    ...
    observations;
  print_endline "trusted-observations=immutable views=complete"

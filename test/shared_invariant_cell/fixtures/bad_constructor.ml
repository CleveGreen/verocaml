...
let bump_and_get (cell : Cell.t @ aliased) : int =
  [%verocaml.requires
    (Cell.model cell).value < 4_611_686_018_427_387_903];
  [%verocaml.ensures fun result ->
    result = [%verocaml.old (Cell.model cell).value] + 1
    && (Cell.model cell).value = result];
  Cell.increment cell;
  Cell.get cell

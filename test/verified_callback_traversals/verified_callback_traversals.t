...
heads rather than merely proving the fixture as a black box.
  ...
  $ printf 'binders int=%s bool=%s parametric=%s option=%s box=%s sequence=%s pair=%s function=%s\n' "$(grep -Ec 'binder=[^ ]+:int qid=' "$sst")" "$(grep -Ec 'binder=[^ ]+:bool qid=' "$sst")" "$(grep -Ec "binder=[^ ]+:'[0-9]+@" "$sst")" "$(grep -Ec 'binder=[^ ]+:option<' "$sst")" "$(grep -Ec 'binder=[^ ]+:box<' "$sst")" "$(grep -Ec 'binder=[^ ]+:sequence<' "$sst")" "$(grep -Ec 'binder=[^ ]+:pair<' "$sst")" "$(grep -Ec 'binder=[^ ]+:\$verocaml.spec-function<' "$sst")"

...
module type STACK = sig
  ...
  val reconstruct : int -> int -> t @ unique
...
module Stack : STACK = struct
  ...
  let reconstruct old_value new_value : t @ unique =
    [%verocaml.ensures fun result ->
      model result = More (new_value, More (old_value, End))];
    let stack =
      {
        top = Node { value = old_value; next = Empty };
        length = 1;
      }
    in
    [%verocaml.proof
      [%verocaml.assert model stack = More (old_value, End)];
      ()];
    stack.top <- Node { value = new_value; next = stack.top };
    stack

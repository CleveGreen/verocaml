type snapshot = End | Fork of int * snapshot * snapshot
...
module Stack : STACK = struct
  ...
  let rec contents_node (node : node) : snapshot =
    ...
    | Empty -> End
    | Node { value; next } ->
        Fork (value, contents_node next, contents_node next)
  [@@verocaml.spec]
  [@@verocaml.opaque]
  ...
  let singleton value : t @ unique =
    [%verocaml.ensures fun result -> model result = Fork (value, End, End)];

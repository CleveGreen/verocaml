...
let prove_model_nonnegative (stack : Stack.t @ read) : unit =
  [%verocaml.ensures fun result -> (Stack.model stack).length >= 0];
  [%verocaml.use_type_invariant stack];
  ()
[@@verocaml.proof]
...
let run value : unit =
  ...
  [%verocaml.proof prove_model_nonnegative stack];
  ()

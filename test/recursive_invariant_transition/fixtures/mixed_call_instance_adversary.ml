...
let mixed_call_instance value (unestablished : Stack.t @ unique) =
  let established = Stack.singleton value in
  let established = Stack.zero_head established in
  let _view = Stack.snapshot established in
  Stack.zero_head unestablished
